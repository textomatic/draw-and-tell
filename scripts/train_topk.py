...
_MODEL_CHECKPOINT = '../models/model_v11_efficientnetb3.pth'
_PLOT_PATH = '../plots'
...
def train_model(dataloaders, dataset_sizes, model, criterion, optimizer, device):
    ...
    Note: validation metric is top-k accuracy where the value of k is defined in the constant `_TOP_K`
        ...
        for phase in ['train', 'test']:
            ...
            for x, y in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    output = model(x)
                    _, topk_preds = torch.topk(output, _TOP_K, dim=1) # Get the index of the top-k highest scored classes
                    topk_preds = topk_preds.t() # Transpose predictions for ease of comparison with y true
                ...
                y_reshaped = y.view(1, -1).expand_as(topk_preds)

                tloss += loss.item() * x.size(0) # Update cumulative loss
                tcorrect += torch.sum(y_reshaped == topk_preds) # Update cumulative correct count
            ...
            tqdm.write(f'{phase.capitalize()} Loss: {epoch_loss:.4f}, Top-{_TOP_K} Acc: {epoch_acc*100:.4f}%')
    ...
    print(f'Best Top-{_TOP_K} Validation Accuracy: {best_test_acc*100:.4f}%')
...
def main():
    """
    Prepares the doodles dataset, trains a neural network model to classify labels of doodle, and evaluates the model performance using top-k accuracy.
    ...
    model = load_model(len(animal_list))
    ...
    # Define optimizer
    optimizer = torch.optim.Adam(model.parameters(), lr=_LEARNING_RATE, amsgrad=True)
